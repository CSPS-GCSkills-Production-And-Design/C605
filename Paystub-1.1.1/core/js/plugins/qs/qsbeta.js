...
function qsBeta(objAcc, paramObj) {
  ...
  if (paramObj === undefined) {
    ...
    var validParamsNames =
      "debugMode,cheatMode,autoStart,encodeAnswers,animBaseDuration,masterIntTester,illegalCharsList,settingsFilename,settingsPath,settingsLabelsFilename,autoTabbing,language";
  ...
  var settingsPath =
    paramObj.settingsPath !== undefined ? paramObj.settingsPath : "settings/";
  var settingsFilename =
    paramObj.settingsFilename !== undefined
      ? paramObj.settingsFilename
      : "qssettings";
  var settingsLabelsFilename =
    paramObj.settingsLabelsFilename !== undefined
      ? paramObj.settingsLabelsFilename
      : "qslabels_";
  ...
  this.qsValidateSettingData = function (sData) {
    ...
    var validSettingsAttributes = {
      ...
      activity: [
        ...
        "final_feedback_score_threshold",
        "submit_all_at_once",
      ...
      type_3: ["ignore_case"],
      type_4: [
        "allow_duplicate_answer",
        "substitute_recap_label",
        "randomize_options",
      ],
      ...
      for (var ii in validSettingsAttributes[i]) {
        if (sData[0][i][validSettingsAttributes[i][ii]] === undefined) {
          invalidSettingsStack +=
            i + " / " + validSettingsAttributes[i][ii] + "\n";
        }
      }
    }
      ...
      404: function () {
        ...
        $.ajax({
          ...
          url:
            ...
            fallbackSettingsLabelsFilename +
            lang +
            ".json",
          dataType: "json",
          async: false,
  ...
  this.settings.debugMode =
    paramObj.debugMode != undefined ? paramObj.debugMode : false;
  this.settings.cheatMode =
    paramObj.cheatMode != undefined ? paramObj.cheatMode : false;
  this.settings.autoStart =
    paramObj.autoStart != undefined ? paramObj.autoStart : true;
  ...
  this.settings.encodeAnswers =
    paramObj.encodeAnswers != undefined ? paramObj.encodeAnswers : false; //EVENTUALLY *OBSOLETE, USE this.settings
  ...
  if (this.settings.cheatMode) {
    ...
    $(".qs-elearning-activity .ra")
      .prop("checked", true)
      .parent()
      .css("border", "1px solid green"); //#raware!
  ...
  //"░▒▓╠╣┌┐║♥♦♠♣";"¼½¾Ð¶¢®©ª¤£¦§±þ§Þß¥º·¹²³¨÷|";
  this.settings.illegalCharsList =
    paramObj.illegalCharsList != undefined
      ? paramObj.illegalCharsList
      : "¼½¾Ð¶¢®©ª¤£¦§±þ§Þß¥º·¹²³¨÷|";
  ...
  this.settings.animBaseDuration =
    paramObj.animBaseDuration != undefined ? paramObj.animBaseDuration : 200;

  //SECURITY "SEED" VALIDATION INTEGER
  this.settings.masterIntTester =
    paramObj.masterIntTester != undefined || paramObj.masterIntTester < 4
      ? paramObj.masterIntTester
      : 33; //KEEP IT OVER 3 AND UNDER 100 PLS! THE HIGHER THE NUMBER THE MORE COMPLEX TO DECODE (FOR END USER SNOOPING AROUND THAT IS!)
  ...
  this.qsInit = function () {
    ...
    if (!this.qsValidateSemantic()) {
      fCe(
        "FAILED TO VALIDATE HTML, PLEASE CONTACT YOUR NETWORK ADMINISTRATOR!"
      );
    }
    ...
    for (
      it = 0;
      it < $(".qs-elearning-activity[data-tracking=true]").length;
      it++
    ) {
      this.qsGetTrackingString(
        $(".qs-elearning-activity[data-tracking=true]").eq(it).attr("id")
      );
    }
    //
    //	I
    ...
    for (i = 0; i < $(".qs-elearning-activity").length; i++) {
      ...
      this.activitiesArr[this.activitiesArr.length] = {
        ...
        activity_type:
          $(currElem).attr("data-activity-type") == undefined
            ? this.objDefVal.activity.activity_type
            : $(currElem).attr("data-activity-type"),
        question_skipping_enabled:
          $(currElem).attr("data-question-skipping") == undefined
            ? this.objDefVal.activity.question_skipping_enabled
            : eval($(currElem).attr("data-question-skipping")), //So users can skip incomplete questions
        tracking_enabled:
          $(currElem).attr("data-tracking") == undefined
            ? this.objDefVal.activity.tracking_enabled
            : eval($(currElem).attr("data-tracking")), //*iLMS tracking answers for this activity...
        recap_required:
          $(currElem).attr("data-force-final-recap") == undefined
            ? this.objDefVal.activity.recap_required
            : eval($(currElem).attr("data-force-final-recap")), //To force recap after activity no matter the feedback type for questions
        ...
        trigger_course_completion:
          $(currElem).attr("data-trigger-scorm-completion") == undefined
            ? eval(this.objDefVal.activity.trigger_course_completion)
            : eval($(currElem).attr("data-trigger-scorm-completion")),
        trigger_completion_score:
          $(currElem).attr("data-trigger-completion-score") != undefined
            ? eval($(currElem).attr("data-trigger-completion-score"))
            : -1,
        final_feedback_score_threshold:
          $(currElem).attr("data-trigger-completion-score") != undefined
            ? eval($(currElem).attr("data-trigger-completion-score"))
            : this.objDefVal.activity.final_feedback_score_threshold, //*TMP, needs data-SOMETHING from DOM eventually *WIP
        ...
        submit_all_at_once:
          $(currElem).attr("data-submit-all-at-once") == undefined
            ? this.objDefVal.activity.submit_all_at_once
            : eval($(currElem).attr("data-submit-all-at-once")),
        submit_aao_disable_right_anwers:
          $(currElem).attr("data-submit-aao-disable-right-answers") == undefined
            ? this.objDefVal.activity.submit_aao_disable_right_anwers
            : eval($(currElem).attr("data-submit-aao-disable-right-answers")),
        simple_act_disable_mode:
          $(currElem).attr("data-simple-act-disable-mode") == undefined
            ? this.objDefVal.activity.simple_act_disable_mode
            : $(currElem).attr("data-simple-act-disable-mode"), //SIMPLE ACTIVITY DISABLE MODE (NEEDS activity_type=simple): 'any'||'onright'||'never' **WIP**
        animations_and_transitions: this.objDefVal.activity
          .animations_and_transitions, //*TMP, needs data-SOMETHING from DOM eventually *WIP
        auto_scroll_to_question: this.objDefVal.activity
          .auto_scroll_to_question, //*TMP, needs data-SOMETHING from DOM eventually *WIP
        force_insert_final_submit: this.objDefVal.activity
          .force_insert_final_submit, //*TMP: G313 EXCEPTION, forces final submit button placement via span/div.qs-final-submit-placeholder, needs data-SOMETHING from DOM eventually *WIP
      ...
      for (ii = 0; ii < $(currElem).find(".qs-exercise").length; ii++) {
        ...
        inherit_question_type =
          $(currElem_ii).attr("data-question-type") === undefined
            ? this.objDefVal.question.question_type
            : $(currElem_ii).attr("data-question-type");
        inherit_feedback_type =
          $(currElem_ii).attr("data-feedback-type") === undefined
            ? this.objDefVal.exercise.feedback_type
            : $(currElem_ii).attr("data-feedback-type");
        inherit_random_answers = eval(
          $(currElem_ii).attr("data-random-answers") === undefined
            ? this.objDefVal.exercise.randomize_answers
            : $(currElem_ii).attr("data-random-answers")
        );
        inherit_score_value =
          $(currElem_ii).attr("data-score-value") === undefined
            ? 1
            : Number($(currElem_ii).attr("data-score-value"));
        //EXERCISES PARAMS
        this.exercisesArr[this.exercisesArr.length] = {
          ...
          exercise_type:
            $(currElem_ii).attr("data-exercise-type") === undefined
              ? this.objDefVal.exercise.exercise_type
              : $(currElem_ii).attr("data-exercise-type"),
          branching_passing_grade: eval(
            $(currElem_ii).attr("data-branching-passing-grade")
          ),
          branching_pass_redirection: $(currElem_ii).attr(
            "data-branching-pass-redirection"
          ),
          branching_fail_redirection: $(currElem_ii).attr(
            "data-branching-fail-redirection"
          ),
          ...
          submit_all_at_once:
            $(currElem_ii).attr("data-submit-all-at-once") == undefined
              ? this.objDefVal.exercise.submit_all_at_once
              : eval($(currElem_ii).attr("data-submit-all-at-once")),
          ...
          pool_picks:
            $(currElem_ii).attr("data-random-questions") == "true"
              ? parseInt($(currElem_ii).attr("data-pool-picks"))
              : "null", //*Choose only X questions from exercise to show, "randomize_questions" has to be true in order to work (duh!)...
        ...
        for (iii = 0; iii < $(currElem_ii).find(".qs-question").length; iii++) {
          ...
          this.questionsArr[nID] = {
            ...
            question_type:
              $(currElem_iii).attr("data-question-type") === undefined
                ? inherit_question_type
                : $(currElem_iii).attr("data-question-type"), //*can be inherited from parent...
            branching_type:
              $(currElem_iii).attr("data-branching-type") === undefined
                ? this.objDefVal.question.branching_type
                : $(currElem_iii).attr("data-branching-type"),
            feedback_type:
              $(currElem_iii).attr("data-feedback-type") === undefined
                ? inherit_feedback_type
                : $(currElem_iii).attr("data-feedback-type"), //*can be inherited from parent... "instant, deferred, none"
            randomize_answers:
              $(currElem_iii).attr("data-random-answers") === undefined
                ? inherit_random_answers
                : eval($(currElem_iii).attr("data-random-answers")), //*can be inherited from parent...
            score_value:
              $(currElem_iii).attr("data-score-value") === undefined
                ? inherit_score_value
                : Number($(currElem_iii).attr("data-score-value")), //*can be inherited from parent...
            dv: new Object({
          ...
          switch (this.questionsArr[nID].question_type) {
            ...
            case "type-1":
              this.questionsArr[
                nID
              ].persistent_vsfb = this.objDefVal.type_1.persistent_vsfb;
              break;
            case "type-2":
              this.questionsArr[
                nID
              ].empty_allowed = this.objDefVal.type_2.empty_allowed;
              break;
            case "type-3":
              this.questionsArr[nID].ignore_case =
                $(currElem_iii).attr("data-question-ignore-case") == undefined
                  ? this.objDefVal.type_3.ignore_case
                  : eval($(currElem_iii).attr("data-question-ignore-case"));

              break;
            case "type-4":
              this.questionsArr[nID].allow_duplicate_answer =
                $(currElem_iii).attr("data-allow-duplicate-answer") == undefined
                  ? this.objDefVal.type_4.allow_duplicate_answer
                  : eval($(currElem_iii).attr("data-allow-duplicate-answer"));
              this.questionsArr[nID].randomize_options =
                $(currElem_iii).attr("data-randomize-options") == undefined
                  ? this.objDefVal.type_4.randomize_options
                  : eval($(currElem_iii).attr("data-randomize-options"));
              break;
            case "type-5":
              break;
            default:
          ...
          switch (this.questionsArr[nID].question_type) {
            case "type-1":
              ...
              if (this.settings.encodeAnswers) {
                for (
                  iQuest = 0;
                  iQuest <
                  $("#" + this.questionsArr[nID].id).find("input.wa, input.ra")
                    .length;
                  iQuest++
                ) {
                  //#raware!
                  this.qsGenerateEncodedValidationString(
                    "gonogo",
                    $("#" + this.questionsArr[nID].id)
                      .find("input.wa, input.ra")
                      .eq(iQuest)
                      .hasClass("ra")
                      ? 1
                      : 0,
                ...
                $(
                  "#" + this.questionsArr[nID].id + ' input[type="radio"]'
                ).removeClass("wa ra"); //REMOVE NOW OBSOLETE WA/RA
              }
              break;
            case "type-2":
              ...
              if (this.settings.encodeAnswers) {
                ...
                $(
                  "#" + this.questionsArr[nID].id + ' input[type="checkbox"]'
                ).removeClass("wa ra"); //REMOVE NOW OBSOLETE WA/RA
              }
              break;
            case "type-3":
              ...
              if (this.settings.encodeAnswers) {
                for (
                  iQuest = 0;
                  iQuest <
                  $("#" + this.questionsArr[nID].id).find('input[type="text"]')
                    .length;
                  iQuest++
                ) {
                  this.qsGenerateEncodedValidationString(
                ...
                $(
                  "#" + this.questionsArr[nID].id + ' input[type="text"]'
                ).removeAttr("data-rep"); //REMOVE NOW OBSOLETE data-rep
              }
              //TMP MODIFIY DOM - STOP USERS FROM SUBMITTING USING ENTER ON TEXT FIELDS
              $("#" + this.questionsArr[nID].id + " input[type=text]").attr(
                "onkeyup",
                "return qsTFRestrictOnKey(this,event);"
              );
              break;
            case "type-4":
              ...
              if (
                this.questionsArr[nID].randomize_options ||
                this.questionsArr[nID].randomize_options === "true"
              ) {
                //$('#'+this.questionsArr[nID].id+' select').each(function(){
                for (
                  rsoi = 0;
                  rsoi < $("#" + this.questionsArr[nID].id + " select").length;
                  rsoi++
                ) {
                  dRep = $("#" + this.questionsArr[nID].id + " select")
                    .eq(rsoi)
                    .attr("data-rep");
                  tmpBuffArr = [];
                  //tmpBuffValArr = [];
                  f1Dummy = false;
                  //SAVE OPTIONS
                  tmpBuffArr = $("#" + this.questionsArr[nID].id + " select")
                    .eq(rsoi)
                    .html()
                    .split("</option>");
                  //REMOVE FROM DOM
                  $("#" + this.questionsArr[nID].id + " select")
                    .eq(rsoi)
                    .empty();
                  ...
                  for (rsoii = 0; rsoii < newMatrixArr.length; rsoii++) {
                    ...
                    $("#" + this.questionsArr[nID].id + " select")
                      .eq(rsoi)
                      .append(tmpBuffArr[newMatrixArr[rsoii]] + "</option>");
                  }
                  ...
                  for (
                    ...
                    if (
                      ...
                      tmpBuff = $("#" + this.questionsArr[nID].id + " select")
                        .eq(rsoi)
                        .find("option")
                        .get(rsoii).outerHTML;
                      $("#" + this.questionsArr[nID].id + " select")
            ...
            case "type-5":
              ...
              if (this.settings.encodeAnswers) {
                ...
                $(
                  "#" + this.questionsArr[nID].id + ' input[type="button"]'
                ).removeClass("wa ra"); //REMOVE NOW OBSOLETE WA/RA
              }
              break;
            default:
          ...
          if (
            this.questionsArr[nID].feedback_type == "deferred" &&
            !this.activities[i].recap_required
          ) {
            this.activities[i].recap_required = this.activities[i].exercises[
              ii
            ].recap_required = true;
          }
          ...
          if (
            this.questionsArr[nID].randomize_answers &&
            (this.questionsArr[nID].question_type == "type-1" ||
              this.questionsArr[nID].question_type == "type-2" ||
              this.questionsArr[nID].question_type == "type-5")
          ) {
            this.qsScrambleAnswers(
              this.questionsArr[nID].id,
              this.questionsArr[nID].question_type
            );
          }
        } //END QUESTION LOOP
        ...
        if (
          this.activities[i].exercises[ii].pool_picks != "null" &&
          iii > this.activities[i].exercises[ii].pool_picks - 1
        ) {
          //REMOVE USELESS QUESTIONS AND REFERENCES OF
          this.activities[i].exercises[ii].questions.splice(
            this.activities[i].exercises[ii].pool_picks,
            this.activities[i].exercises[ii].nb_questions -
              this.activities[i].exercises[ii].pool_picks
          );
          ...
          this.activities[i].exercises[ii].nb_questions = this.activities[
            i
          ].exercises[ii].questions.length;
    ...
    for (iqn = 0; iqn < this.activities.length; iqn++) {
      for (iiqn = 0; iiqn < this.activities[iqn].exercises.length; iiqn++) {
        if (
          this.activities[iqn].exercises[iiqn].randomize_questions == "true"
        ) {
          //IF RANDOM
          tmpNH = this.activities[iqn].exercises[iiqn].random_order.split(",");
          for (
            iiiqn = 0;
            iiiqn < this.activities[iqn].exercises[iiqn].pool_picks;
            iiiqn++
          ) {
            //POOL PICKS SHOULD BE MAX
            $(
              "#" +
                this.activities[iqn].exercises[iiqn].questions[iiiqn].id +
                " .qs-insert-question-number"
            ).html(tmpCounter); //QUESTION NUMBER
            $(
              "#" +
                this.activities[iqn].exercises[iiqn].questions[iiiqn].id +
                " .qs-insert-question-value"
            ).html(
              this.msgs.msgPtsPrefix +
                this.activities[iqn].exercises[iiqn].questions[iiiqn]
                  .score_value +
                this.msgs.msgPtsSuffix
            ); //QUESTION VALUE
            tmpCounter++;
          }
        } else {
          ...
          for (
            iiiqn = 0;
            iiiqn < this.activities[iqn].exercises[iiqn].questions.length;
            iiiqn++
          ) {
            //NO NEED TO GO THROUGH EVERYTHING
            $(
              "#" +
                this.activities[iqn].exercises[iiqn].questions[iiiqn].id +
                " .qs-insert-question-number"
  ...
  this.qsPrepActivities = function (activity_id) {
    ...
    //ASSIGN STEP CLASS
    $("#" + this.activities[activity_id].id + ".qs-elearning-activity")
      .removeClass("qs-step2 qs-step3")
      .addClass("qs-step1");

    //MEGA Hide stuff... (TMP)
    qsHide(
      $(this.activities[activity_id].elem).find(
        ".qs-subvisit, .qs-question, .qs-exercise, .qs-feedback, .qs-specific, .qs-second-chance, .qs-feedback-final, .qs-finallinkto-unanswered-placeholder"
      )
    );
    ...
    if (
      $("#" + this.activities[activity_id].id + " .qs-start-activity").length
    ) {
      //MEGA SHOW STUFF (TMP)
      qsShow("#" + this.activities[activity_id].id + " .qs-start-activity");
      fCl(
        "FIND ME (in qsPrepActivities()) and hide stuff via CSS for added security and compatibility"
      );
      ...
      if (this.activities[activity_id].dv.status != "unattempted") {
        ...
        $(
          "#" +
            this.activities[activity_id].id +
            " .qs-start-activity .qs-subvisit .qs-resetretry"
        ).unbind("click");
        $(
          "#" +
            this.activities[activity_id].id +
        ...
        $(
          "#" +
            this.activities[activity_id].id +
            " .qs-start-activity .qs-subvisit .qs-getbacktoit"
        ).click(function () {
          if (!$(this).hasClass("qs-disabled")) {
            fCl(
            ...
            $(this).unbind();
            mThis.qsActivityInitialLaunch(
              $(this).closest("div.qs-elearning-activity").attr("id")
            );
          }
        });
      }
      ...
      $(this.activities[activity_id].elem)
        .find(".qs-start-activity .qs-start")
        .click(function () {
          if (!$(this).hasClass("qs-disabled")) {
            fCl(
              "---------------------------- PRESSED START ----------------------------"
            ...
            $(this).unbind();
            var activity_stringID = $(this)
              .closest("div.qs-elearning-activity")
              .attr("id");

            //FORCE RESTART AT EX=0 - Q=0
            mThis.activities[
              mThis.qsGetActivityIDFromStringID(activity_stringID)
            ].dv.current_exercise_id = 0;
            mThis.activities[
              mThis.qsGetActivityIDFromStringID(activity_stringID)
            ].dv.current_question_id = 0;
  ...
  this.qsActivityInitialLaunch = function (activity_stringID) {
    ...
    current_exercise_ID = this.activities[
      this.qsGetActivityIDFromStringID(activity_stringID)
    ].dv.current_exercise_id;
    current_question_ID = this.activities[
      this.qsGetActivityIDFromStringID(activity_stringID)
    ].dv.current_question_id;

    current_question_stringID = this.activities[current_activity_ID].exercises[
      current_exercise_ID
    ].questions[current_question_ID].id;

    //BRANCHING QUIZZES
    if (this.activities[current_activity_ID].activity_type === "branching") {
      fCe(
        'BRANCHING QUIZZES, WIP! -> Tracking not work correctly, recap not working, lots of options not working in conjonction with activity-type="branching"'
      );
    }

    //ASSIGN STEP CLASS
    $("#" + activity_stringID + ".qs-elearning-activity")
      .removeClass("qs-step1 qs-step3")
      .addClass("qs-step2");
    ...
    ancestrySTRARR = this.qsGetAncestryStringIDS(
      $("#" + current_question_stringID)
    );

    //HIDE .qs-start-activity/SHOW .qs-exercise IF NECESSARY
    if ($("#" + activity_stringID + " .qs-start-activity").length) {
      qsTransition(
        $("#" + activity_stringID + " .qs-start-activity"),
        $("#" + activity_stringID + " #" + ancestrySTRARR[1] + ".qs-exercise")
      );
    } else {
    ...
    this.activities[current_activity_ID].exercises[
      current_exercise_ID
    ...
    this.activities[current_activity_ID].exercises[
      ...
      this.activities[current_activity_ID].exercises[current_exercise_ID].dv
        .status === "unattempted"
        ? "incomplete"
        : this.activities[current_activity_ID].exercises[current_exercise_ID].dv
            .status;
    ...
    $(
      "#" +
        ...
        activity_stringID +
        " .qs-nextquestion"
    ).removeClass("qs-disabled");

    //DISABLE PREV QUESTION IF ON FIRST QUESTION
    if (
      this.activities[current_activity_ID].dv.current_exercise_id === 0 &&
      this.activities[current_activity_ID].dv.current_question_id === 0
    ) {
    ...
    $(
      ...
      if (
        !$(this).hasClass("qs-disabled") &&
        !$(this).hasClass("qs-tmp-disabled")
      ) {
        fCl(
          "---------------------------- PRESSED NEXT/BACK ----------------------------"
        );
  ...
  this.qsSwitchExercise = function (
    current_activity_id,
    current_exercise_id,
    direction
  ) {
    ...
    if (
      (current_exercise_id + direction < 0 ||
        current_exercise_id + 1 + direction >
          this.activities[current_activity_id].nb_exercises) &&
      !(
        this.activities[current_activity_id].exercises[current_exercise_id]
          .exercise_type === "branching-score-redirect" ||
        this.activities[current_activity_id].exercises[current_exercise_id]
          .exercise_type === "branching-selective-redirect"
      )
    ) {
      ...
      if (
        ...
        if (
          ...
          //*WIP THIS IS NOT COOL, LOOK AT THE PREVIOUS IF (SAME_THING)!
          //
          ...
          for (
            ...
            if (
              this.activities[current_activity_id].exercises[
                current_exercise_id
              ].questions[i].dv.status === "completed"
            ) {
              exNaturalScore += 1;
            }
          }
          ...
          if (
            exNaturalScore >=
            this.activities[current_activity_id].exercises[current_exercise_id]
              .branching_passing_grade
          ) {
            gotoRedirection = this.activities[current_activity_id].exercises[
              current_exercise_id
            ].branching_pass_redirection;
          } else {
            gotoRedirection = this.activities[current_activity_id].exercises[
              current_exercise_id
            ].branching_fail_redirection;
          }
        } else if (
          this.activities[current_activity_id].exercises[current_exercise_id]
            .exercise_type === "branching-selective-redirect"
          ...
          //
          fCl(
            "CHECK IF CORRECT Q TYPE BEFORE GOING ANY FURTHER, SHOULD BE DONE IN PRE LOADING PHASES OR qsValidateSemantic() + CAN'T HAVE RANDOM RIGHT NOW AS THE LAST QUESTION SHOULD ALWAYS BE THE SELECTIVE REDIRECTION ONE"
          );
          gotoRedirection = $(
            "#" +
              this.activities[current_activity_id].exercises[
          ...
          ).attr("data-selective-redirection");
        ...
        if (gotoRedirection.indexOf("f|") === 0) {
          qsHide(
            "#" +
              this.activities[current_activity_id].id +
              " .qs-feedback-final .qs-final-branching-feedback"
          );
          qsShow(
            "#" +
              this.activities[current_activity_id].id +
              " .qs-feedback-final #" +
              gotoRedirection.substring(2, gotoRedirection.length) +
              ".qs-final-branching-feedback"
          );
          fCe(
            "SHOW -> #" + gotoRedirection.substring(2, gotoRedirection.length)
          );
          return false;
        }
        //GET FAKE "DIRECTION"
        nextExerciseID = this.qsGetExerciseIDFromStringID(
          gotoRedirection,
          current_activity_id
        );
      ...
      //SETS DV
      this.activities[current_activity_id].dv.current_exercise_id =
        current_exercise_id + direction;
      this.activities[
        current_activity_id
      ].dv.current_question_id = next_question_id;

      //IF FIRST TIME SEEING QUESTION, MARK IT INCOMPLETE (AS OPPOSED TO UNATTEMPTED)
      this.activities[current_activity_id].exercises[
        current_exercise_id + direction
      ].questions[next_question_id].dv.status =
        this.activities[current_activity_id].exercises[
          current_exercise_id + direction
        ].questions[next_question_id].dv.status == "unattempted"
          ? "incomplete"
          : this.activities[current_activity_id].exercises[
              current_exercise_id + direction
            ].questions[next_question_id].dv.status;
  ...
  this.qsSwitchQuestion = function (elem, qNum) {
    ...
    current_activity = this.qsGetActivityIDFromStringID(
      $(elem).closest(".qs-elearning-activity").attr("id")
    );
    ...
    qsHide(
      "#" +
        ...
        " .qs-finallinkto-unanswered-placeholder"
    );
    ...
    for (i = 0; i < this.activities[current_activity].nb_exercises; i++) {
      qMatrixArr[i] = this.activities[current_activity].exercises[
        i
      ].nb_questions;
      ...
      if (qNum - 1 < qAt && target_exercise_STRID === "") {
        ...
        target_exercise = i;
        target_exercise_STRID = this.activities[current_activity].exercises[i]
          .id;
      }
    }
    ...
    if (current_exercise == target_exercise) {
      ...
      qsTransition(
        "#" +
          this.activities[current_activity].exercises[current_exercise]
            .questions[current_question].id,
        "#" +
          this.activities[current_activity].exercises[current_exercise]
            .questions[target_question].id
      );
    } else {
      ...
      qsShow(
        "#" +
          target_exercise_STRID +
          " #" +
          this.activities[current_activity].exercises[target_exercise]
            .questions[target_question].id
      );
      //TRANSITION FROM CURRENT QUESTION/EXERCISE TO TARGET QUESTION/EXERCISE
      qsTransition(
        "#" + this.activities[current_activity].exercises[current_exercise].id,
        "#" + this.activities[current_activity].exercises[target_exercise].id
      );
    }
  ...
  this.qsNavQuestions = function (elem) {
    ...
    current_exercise_ID = this.activities[current_activity].dv
      .current_exercise_id;
    current_question_ID = this.activities[current_activity].dv
      .current_question_id;

    //STOP USERS FROM GOING BACK WHEN ALREADY AT FIRST QUESTION
    if (
      this.activities[current_activity].dv.current_exercise_id === 0 &&
      this.activities[current_activity].dv.current_question_id === 0 &&
      $(elem).hasClass("qs-prevquestion")
    ) {
      fCl("cannot go back!");
      return false;
    }
    ...
    if (
      !this.activities[current_activity].question_skipping_enabled &&
      $(elem).hasClass("qs-nextquestion")
    ) {
      if (
        this.activities[current_activity].exercises[current_exercise_ID]
          .questions[current_question_ID].dv.status == "unattempted" ||
        this.activities[current_activity].exercises[current_exercise_ID]
          .questions[current_question_ID].dv.status == "incomplete"
      ) {
        fCl(
          "Cant go to the next question without answering the current first, please answer current question or contact your nextwork administrator"
        );
    ...
    $("#" + this.activities[current_activity].id)
      .find(".qs-prevquestion, .qs-nextquestion")
      .addClass("qs-tmp-disabled"); //DISABLE BOTH
    ...
    setTimeout(function () {
      ...
      $("#" + $(elem).closest(".qs-elearning-activity").attr("id"))
        .find(".qs-prevquestion, .qs-nextquestion")
        .removeClass("qs-tmp-disabled"); //DISABLE BOTH
  ...
  this.qsNextQuestion = function (elem, qOffset, excpt) {
    ...
    nb_question = this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
      .nb_questions;

    //DESTROY .qs-finallinkto-unanswered SINCE NAVIGATING AWAY
    $(
      "#" + this.activities[ancestryIDARR[0]].id + " .qs-finallinkto-unanswered"
    ).remove();

    if (excpt !== "final-submit") {
      //IGNORE ON FINAL-SUBMIT SINCE ITS NOT CHANGING QUESTION
      //DISABLE QS-PREVQUESTION IF CURRENT QUESTION IS FIRST
      //if(current_exercise === 0 && current_question+qOffset === 0){
      if (
        eval(this.qsComputeProgression(ancestryIDARR[0], "qat")) + qOffset ===
        1
      ) {
        $(
          "#" + this.activities[ancestryIDARR[0]].id + " .qs-prevquestion"
        ).addClass("qs-disabled");
      } else {
        $(
          "#" + this.activities[ancestryIDARR[0]].id + " .qs-prevquestion"
      ...
      if (
        eval(this.qsComputeProgression(ancestryIDARR[0], "qat") + qOffset) >=
        eval(this.qsComputeProgression(ancestryIDARR[0], "qtot"))
      ) {
        //--> ADD EXCEPTION TO ALLOW NEXT ON LAST QUESTION TO COMPLETE ACTIVITY (GOTO STEP 3)
        $(
          "#" + this.activities[ancestryIDARR[0]].id + " .qs-nextquestion"
        ).addClass("qs-disabled"); //FIX NEXT BT TO COMPLETE ACTIVITY (FINAL SUBMIT)
      } else {
        $(
          "#" + this.activities[ancestryIDARR[0]].id + " .qs-nextquestion"
        ).removeClass("qs-disabled");
...
current_question = 3;*/
    ...
    if (
      this.activities[ancestryIDARR[0]].submit_all_at_once &&
      eval(this.qsComputeProgression(ancestryIDARR[0], "qat") + 1) ==
        eval(this.qsComputeProgression(ancestryIDARR[0], "qtot")) &&
      qOffset === 1
    ) {
      ...
      if (
        !$("#" + this.activities[ancestryIDARR[0]].id).find(
          ".qs-submit-activity"
        ).length
      ) {
        //NOT FOUND, ADD IT!
        ...
        if (
          ...
          $(
            "#" +
              this.activities[ancestryIDARR[0]].id +
              " .qs-final-submit-placeholder"
          ).append(this.objDefVal.domButtons.qs_submit_activity);
        } else {
          ...
          $(
            "#" +
              this.activities[ancestryIDARR[0]].exercises[
                this.activities[ancestryIDARR[0]].nb_exercises - 1
              ].questions[
                this.activities[ancestryIDARR[0]].exercises[
                  this.activities[ancestryIDARR[0]].nb_exercises - 1
                ].nb_questions - 1
    ...
    if (
      current_question + qOffset >= nb_question ||
      current_question + qOffset < 0
    ) {
      //GOING TO NEXT/PREVIOUS EXERCISE! MAYBE QUIZ COMPLETED?
      fCl(
        "Going outside of current exercise: " +
          current_question +
          "+" +
          qOffset +
          "=" +
          (current_question + qOffset)
      );
      ...
      this.activities[ancestryIDARR[0]].dv.current_question_id =
        current_question + qOffset;
      ...
      //IF FIRST TIME SEEING QUESTION, MARK IT INCOMPLETE (AS OPPOSED TO UNATTEMPTED)
      this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].questions[
        current_question + qOffset
      ].dv.status =
        this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].questions[
          current_question + qOffset
          ...
          ? "incomplete"
          : this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[current_question + qOffset].dv.status;
      //MARK EXERCISE AS INCOMPLETED IF UNATTEMPTED
      this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].dv.status =
        this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].dv
          .status === "unattempted"
          ? "incomplete"
          : this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].dv
              .status;
      ...
      qsTransition(
        ...
        "#" +
          this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[current_question + qOffset].id
      );
  ...
  this.qsAnswerQuestion = function (elem, qMode) {
    ...
    ancestryIDARR = this.qsGetAncestryIDS(elem);
    question_type = this.activities[ancestryIDARR[0]].exercises[
      ancestryIDARR[1]
    ].questions[ancestryIDARR[2]].question_type;
    ...
    switch (question_type) {
      ...
      case "type-0":
        //CHECK IF NO ANSWER
        fCe(
          "type-0: Needs to replace reserved chars...right now it doesnt...catastrophic failure can and will happen! FIX ME"
        );

        right_answer = true;
        $(
          this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[ancestryIDARR[2]].answers.elem
        )
          .find("textarea")
          .each(function () {
            if (
        ...
        if (
          (qMode == "final-submit-all-at-once" || qMode == "final-submit") &&
          !this.activities[ancestryIDARR[0]].submit_aao_disable_right_anwers
        ) {
          //NOTHING FOR NOW...
        } else if (
          this.activities[ancestryIDARR[0]].activity_type === "simple" &&
          this.activities[ancestryIDARR[0]].simple_act_disable_mode === "never"
        ) {
          //DON'T DISABLE
        } else {
          $(
        ...
        this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].questions[
          ancestryIDARR[2]
        ].dv.my_answer_id = saveBuff.join(qSplt[1]);
        break;
      //
      //  TYPE-1
      //
      ...
      case "type-1":
        ...
        raCheck = this.settings.encodeAnswers
          ? this.qsTestAgainstEncodedValidationString(
              "gonogo",
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                  .questions[ancestryIDARR[2]].answers.elem
        ...
        if (raCheck) {
          ...
          if (
            ...
            this.activities[ancestryIDARR[0]].activity_type === "simple" &&
            this.activities[ancestryIDARR[0]].simple_act_disable_mode ===
              "never"
          ) {
            //DON'T DISABLE
          } else {
            ...
            $(
              ...
              .parent()
              .find("input.qs-submit")
              .attr("disabled", true); //CSPS-SJ disable valider
          }
        } else {
          //WRONG ANSWER
          answerStatus = "wrong";
          ...
          if (
            this.qsManageSecondChance(
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].id
            )
          ) {
        ...
        qsShow(
          $(
            "#" +
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].id +
              " .qs-feedback .qs-specific[data-specific-feedback-target=" +
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                  .questions[ancestryIDARR[2]].elem
              )
                .find('input[type="radio"]:checked')
                .attr("id") +
              "]"
        ...
        for (
          ...
          tElem = this.qsGetClosestMatch(
            ".qs-vsfbh",
            $(
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].answers.elem
            ).find('input[type="radio"]:checked')[vsfbhi].id
          );
          ...
          raCheck = this.settings.encodeAnswers
            ? this.qsTestAgainstEncodedValidationString(
                ...
                $(
                  ...
                  ).find('input[type="radio"]:checked')[vsfbhi]
                ).attr("data-encoded-rep")
              )
            : $(
                $(
      ...
      case "type-2":
        ...
        if (
          ...
          !this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[ancestryIDARR[2]].empty_allowed
        ) {
          if (qMode !== "final-submit-all-at-once") {
        ...
        $(
          ...
          .each(function () {
            raCheck = mThis.settings.encodeAnswers
              ? mThis.qsTestAgainstEncodedValidationString(
                  "gonogo",
                  $(this).attr("data-encoded-rep")
                ) != $(this).prop("checked")
              : $(this).hasClass("ra") != $(this).prop("checked");
        ...
        $(
          ...
          .each(function (ind, val) {
            qsShow(
              $(
                "#" +
                  mThis.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                    .questions[ancestryIDARR[2]].id +
                  " .qs-feedback .qs-specific[data-specific-feedback-target=" +
                  $(this).attr("id") +
                  "]"
        ...
        for (
          ...
          tElem = this.qsGetClosestMatch(
            ...
            $(
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].answers.elem
            ).find('input[type="checkbox"]')[vsfbhi].id
          ); //<-HIGHLITE ALL MODE
          ...
          raCheck = this.settings.encodeAnswers
            ? this.qsTestAgainstEncodedValidationString(
                ...
                $(
                  ...
                  ).find('input[type="checkbox"]')[vsfbhi]
                ).attr("data-encoded-rep")
              )
            : $(
                $(
          ...
          if (raCheck) {
            if (
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                  .questions[ancestryIDARR[2]].answers.elem
              ).find('input[type="checkbox"]')[vsfbhi].checked
            ) {
              $(tElem).html(this.objDefVal.domVisuals.positive_feedback);
            } else {
              $(tElem).html(this.objDefVal.domVisuals.negative_feedback);
      ...
      case "type-3":
        ...
        $(
          ...
          .each(function () {
            ...
            var testAgainst = mThis.settings.encodeAnswers
              ? mThis.qsTestAgainstEncodedValidationString(
                  "encrypt",
                  $(this).attr("data-encoded-rep")
                )
            ...
            var isIgnoringCase =
              mThis.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].ignore_case;
            ...
            if (testAgainst.split("|").length > 1) {
              ...
              for (
                iMulti = 0;
                iMulti < testAgainst.split("|").length;
                iMulti++
              ) {
        ...
        for (
          ...
          tElem = this.qsGetClosestMatch(
            ...
            $(
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].answers.elem
            ).find('input[type="text"]')[vsfbhi].id
          );
          if (
            $(
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                  .questions[ancestryIDARR[2]].answers.elem
              ).find('input[type="text"]')[vsfbhi]
            ).hasClass("qs_good_answer")
          ) {
            $(tElem).html(this.objDefVal.domVisuals.positive_feedback);
          } else {
            $(tElem).html(this.objDefVal.domVisuals.negative_feedback);
      ...
      case "type-4":
        ...
        if (
          this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[ancestryIDARR[2]].allow_duplicate_answer == false &&
          $(
            this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[ancestryIDARR[2]].answers.elem
        ...
        for (i = 0; i < dDAnswerArr.length; i++) {
          ...
          qsShow(
            $("#" + dDAnswerArr[i][0])
              .closest(".qs-answers")
              .find(".qs-vsfbh")
          ); //*WIP NEEDS TO BE OUTSIDE OF LOOP
        ...
        for (i in dDAnswerArr) {
          ...
          if (
            ...
            if (answerStatus === "right") {
              $("select#" + dDAnswerArr[i][0]).prop("disabled", "disabled");
            } else if (dDAnswerArr.length > 1 && dDAnswerArr[i][1]) {
              //CSPS-SJ
          ...
          if (
            $("#" + dDAnswerArr[i][0]).attr(
              "data-specific-negative-feedback"
            ) !== undefined &&
            dDAnswerArr[i][1] === false
          ) {
            qsShow(
              $(
                "#" +
          ...
          if (
            $("#" + dDAnswerArr[i][0]).attr(
              "data-specific-positive-feedback"
            ) !== undefined &&
            dDAnswerArr[i][1] === true
          ) {
            qsShow(
              $(
                "#" +
      ...
      case "type-5":
        ...
        for (
          ...
          if (
            $(
              this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                .questions[ancestryIDARR[2]].answers.elem
            ).find('input[type="button"]')[vsfbhi].id === $(elem).attr("id")
          ) {
            $(
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
                  .questions[ancestryIDARR[2]].answers.elem
              ).find('input[type="button"]')[vsfbhi]
            ).addClass("qs-answered");
          } else {
            $(
              $(
                this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
        ...
        for (
          ...
          $(
            this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[ancestryIDARR[2]].answers.elem
          ).find('input[type="button"][id="' + $(elem).attr("id") + '"]')
            .length;
          vsfbhi++
        ) {
          //<-HIGHLITE ONLY CHECKED
            ...
            ? this.qsTestAgainstEncodedValidationString(
                ...
                $(
                  $(
                    ...
                    'input[type="button"][id="' + $(elem).attr("id") + '"]'
                  )[vsfbhi]
                ).attr("data-encoded-rep")
              )
            : $(
    ...
    if (answerStatus === "right") {
      ...
      qsShow(
        $(
          this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[ancestryIDARR[2]].elem
        ).find(".qs-feedback .qs-right, .qs-feedback .qs-generic")
      );
      qsHide(
        $(
          this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
            .questions[ancestryIDARR[2]].elem
        ).find(".qs-feedback .qs-wrong")
      );
      ...
      if (secondChanceNeeded) {
        qsHide(
          $(
            ...
            ".qs-feedback .qs-wrong, .qs-feedback .qs-right, .qs-feedback .qs-generic"
          )
        );
        qsShow(
          $(
            this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[ancestryIDARR[2]].elem
          ).find(".qs-feedback .qs-second-chance")
        );
      } else {
        //
        //DISPLAY FEEDBACK RIGHT AWAY IF NEEDED (HIDE OPPOSITE FEEDBACK)
        qsShow(
          $(
            this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[ancestryIDARR[2]].elem
          ).find(".qs-feedback .qs-wrong, .qs-feedback .qs-generic")
        );
        qsHide(
          $(
            this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]]
              .questions[ancestryIDARR[2]].elem
          ).find(".qs-feedback .qs-right")
        );
    ...
    if (!secondChanceNeeded) {
      ...
      questionFeedbackType = this.activities[ancestryIDARR[0]].exercises[
        ancestryIDARR[1]
      ].questions[ancestryIDARR[2]].feedback_type;

      if (
        questionFeedbackType == "deferred" ||
        questionFeedbackType == "none"
      ) {
        ...
        if (qMode === "default") {
          ...
          fCe(
            '*WIP !IMPORTANT! PLEASE FIX FORCED HARDCODED INTEGER "123456"!!!'
          );
        ...
        C = $(this.activities[ancestryIDARR[0]].elem).find(".qs-feedback-final")
          .length; //IS THERE A FINAL FEEDBACK PHASE
        D =
          this.qsComputeProgression(ancestryIDARR[0], "qat") !==
          this.qsComputeProgression(ancestryIDARR[0], "qtot"); //IS THIS THE LAST QUESTION
          ...
          if (!C && !D) {
            ...
            if (this.activities[ancestryIDARR[0]].force_insert_final_submit) {
              ...
              $("#" + this.activities[ancestryIDARR[0]].id)
                ...
                .html(this.objDefVal.domButtons.qs_submit_activity);
              ...
              $("#" + this.activities[ancestryIDARR[0]].id)
                ...
                .click(function () {
                  if (!$(this).hasClass("qs-disabled")) {
                    fCl(
                      "---------------------------- PRESSED CONTINUE FROM FEEDBACK BOX AFTER ANSWERING QUESTION (CREATED VIA qsAnswerQuestion()) ----------------------------"
                    );
                    //mThis.qsNextQuestion(this);

                    //GOOD ONE->//mThis.qsNextQuestion($('#'+$(this).attr('data-pass-id')));
                    //FORCE FINAL SUBMIT
                    mThis.qsNextQuestion(
                      $("#" + $(this).attr("data-pass-id")),
                      1,
                      "final-submit"
                    );
            ...
            $(elem)
              .closest(".qs-question")
              .find(".qs-feedback")
              .append(this.objDefVal.domButtons.qs_continue);
  ...
  this.qsMarkExerciseDone = function (activity_id, exercise_id) {
    ...
    if (
      this.activities[activity_id].exercises[exercise_id].dv.status ==
        "completed" ||
      this.activities[activity_id].exercises[exercise_id].dv.status == "failed"
    ) {
      return false;
    } else {
      allGood = true;
      ...
      for (
        i = 0;
        i < this.activities[activity_id].exercises[exercise_id].nb_questions;
        i++
      ) {
        if (
          this.activities[activity_id].exercises[exercise_id].questions[i].dv
            .status != "completed" &&
          this.activities[activity_id].exercises[exercise_id].questions[i].dv
            .status != "failed"
        ) {
      ...
      if (allGood) {
        ...
        $(
          "#" +
            this.activities[activity_id].exercises[exercise_id].id +
            ".qs-question"
        ...
        for (i = 0; i < this.activities[activity_id].nb_exercises; i++) {
          if (
            this.activities[activity_id].exercises[i].dv.status != "completed"
          ) {
            allGood_b = false;
            break;
          }
        ...
        if (allGood_b) {
          ...
          $("#" + this.activities[activity_id].id + ".qs-question").attr(
            "data-status",
            "completed"
          );
  ...
  this.qsActivityCompleted = function (elem) {
    ...
    if (this.activities[ancestryIDARR[0]].submit_all_at_once) {
      ...
      for (i = 0; i < this.activities.length; i++) {
        for (ii = 0; ii < this.activities[i].exercises.length; ii++) {
          for (
            iii = 0;
            iii < this.activities[i].exercises[ii].questions.length;
            iii++
          ) {
            if (
              this.activities[i].exercises[ii].questions[iii].dv.status !==
                "failed" ||
              this.activities[i].exercises[ii].questions[iii].dv.status !==
                "completed"
            ) {
              this.qsAnswerQuestion(
                this.activities[i].exercises[ii].questions[iii].elem,
                "final-submit-all-at-once"
              ); //DISABLE THE ALERTS
            }
          }
    ...
    if (this.activities[ancestryIDARR[0]].trigger_course_completion) {
      ...
      fCe(
        "COURSE COMPLETION VALIDATION:" +
          this.qsComputeActivityScore(ancestryIDARR[0], "perc") +
          "% (" +
          this.activities[ancestryIDARR[0]].trigger_completion_score +
          "% OR MORE NEEDED TO GRADUATE)\nCOURSE COMPLETION TRIGGER IS CURRENTLY INCOMPLETE AND WILL NOT SAVE ANYTHING TO THE LMS"
      );
      if (
        this.qsComputeActivityScore(ancestryIDARR[0], "perc") >=
        this.activities[ancestryIDARR[0]].trigger_completion_score
    ...
    if (
      ...
      !this.activities[ancestryIDARR[0]].recap_required
    ) {
      //
      //SAVE DATA
      ...
      fCl(
        "RECREATE MISSING .qs-feedback-final + qs-recap-template + buttons....*WIP"
      );
    }
    ...
    if (this.activities[ancestryIDARR[0]].activity_type !== "branching") {
      ...
      for (
        ii = 0;
        ii < this.activities[ancestryIDARR[0]].exercises.length;
        ii++
      ) {
        for (
          iii = 0;
          iii <
          this.activities[ancestryIDARR[0]].exercises[ii].questions.length;
          iii++
        ) {
          masterCounter += 1;
          if (
            this.activities[ancestryIDARR[0]].exercises[ii].questions[iii].dv
              .status !== "completed" &&
            this.activities[ancestryIDARR[0]].exercises[ii].questions[iii].dv
              .status !== "failed"
          ) {
      ...
      if (!allQuestionsAnswered) {
        ...
        for (i = 0; i < unansweredArr.length; i++) {
          ...
          linksBuffer +=
            ...
            objAcc +
            ".qsSwitchQuestion('#" +
            $(elem).closest(".qs-question").attr("id") +
            "'," +
            unansweredArr[i].split(",")[0] +
            ');return false;">' +
            this.msgs.msgUnanweredQuestionGotoPrefix +
            unansweredArr[i].split(",")[0] +
            this.msgs.msgUnanweredQuestionGotoSuffix +
            "</a></li>";
        }
        linksBuffer += "</ul></div>";
        ...
        if (
          ...
          $(
            "#" + this.activities[ancestryIDARR[0]].id + " .qs-submit-activity"
          ).after(linksBuffer);
        }
        //SHOW LIST
        qsShow(
    ...
    $("#" + this.activities[ancestryIDARR[0]].id + " .qs-question")
      .find(
    ...
    //ASSIGN STEP CLASS
    $("#" + this.activities[ancestryIDARR[0]].id + ".qs-elearning-activity")
      .removeClass("qs-step1 qs-step2")
      .addClass("qs-step3");
    ...
    var recapTemplateElem = $(
      "#" +
        this.activities[ancestryIDARR[0]].id +
        " .qs-feedback-final .qs-recap-template"
    );
    $(recapTemplateElem).find(".qs-recap-item").remove();
    if (
      this.activities[ancestryIDARR[0]].recap_required &&
      $(recapTemplateElem).length
    ) {
      ...
      var iCounter = 0;
      fCl(
        "currently not ready for multiple activities on page (loose selectors) *TMP"
      );
      for (i = 0; i < this.activities[ancestryIDARR[0]].exercises.length; i++) {
        for (
          ii = 0;
          ii < this.activities[ancestryIDARR[0]].exercises[i].questions.length;
          ii++
        ) {
          iCounter++;
          appendBuffer = "";
          recapItemID =
            "qs-recap-item-" +
            this.activities[ancestryIDARR[0]].exercises[i].questions[ii].id;
          appendBuffer +=
            "<div class='qs-post-question'>" +
            $(this.activities[ancestryIDARR[0]].exercises[i].questions[ii].elem)
              .find(".qs-text")
              .html() +
            "</div>";
          ...
          switch (
            this.activities[ancestryIDARR[0]].exercises[i].questions[ii]
              .question_type
          ) {
            case "type-0":
              appendBuffer +=
                "<div class='qs-post-answers'>" +
                this.msgs.msgFeedbackRecallAnswerPrefix;
              for (
                iii = 0;
                iii <
                this.activities[ancestryIDARR[0]].exercises[i].questions[
                  ii
                ].dv.my_answer_id.split(qSplt[1]).length;
                iii++
              ) {
                appendBuffer +=
                  this.activities[ancestryIDARR[0]].exercises[i].questions[
            ...
            case "type-2":
              ...
              if (
                this.activities[ancestryIDARR[0]].exercises[i].questions[ii].dv
                  .my_answer_id !== "" &&
                this.activities[ancestryIDARR[0]].exercises[i].questions[ii].dv
                  .my_answer_id !== undefined
              ) {
                  ...
                  appendBuffer +=
                    $(
                      "label[for=" +
                        this.activities[ancestryIDARR[0]].exercises[
                          i
                        ].questions[ii].dv.my_answer_id.split(",")[iii] +
                        "]"
                    ).text() +
                    (this.activities[ancestryIDARR[0]].exercises[i].questions[
                      ii
                  ...
                  $(
                    "#" +
                      this.activities[ancestryIDARR[0]].exercises[i].questions[
                        ...
                        .split(qSplt[1])
                        [iii].split(qSplt[0])[0]
                  ).attr("data-recap-text") !== undefined
                ) {
                  appendBuffer +=
                    ": " +
                    $(
                      "#" +
                        this.activities[ancestryIDARR[0]].exercises[
                          i
                        ].questions[ii].dv.my_answer_id
                          .split(qSplt[1])
                          [iii].split(qSplt[0])[0]
                    ).attr("data-recap-text");
                } else if (
                  $(
                    "#" +
                      this.activities[ancestryIDARR[0]].exercises[i].questions[
                        ...
                        .split(qSplt[1])
                        [iii].split(qSplt[0])[0]
                  ).attr("data-recap-label") !== undefined
                ) {
                  appendBuffer +=
                    ": " +
                    $(
            ...
            default:
              fCe(
                "Recap feedback generation encountered unknown question type"
              );
          }
          ...
          $(recapTemplateElem)
            .find(".qs-get-current-question")
            .removeClass("qs-get-current-question")
            .html(iCounter);
          //console.error($(recapTemplateElem).find('.qs-get-current-question'));
          $("#" + recapItemID + " .qs-post-feedback")
            .find("a, input[type=button]")
            .remove();
        }
      }
    ...
    qsShowSequentially(
      "#" + this.activities[ancestryIDARR[0]].id + " .qs-post-question"
    );
    ...
    qsTransition(
      ...
      "#" + this.activities[ancestryIDARR[0]].id + " .qs-feedback-final"
    );
    ...
    if (
      this.qsComputeActivityScore(ancestryIDARR[0], "perc") >=
      this.activities[ancestryIDARR[0]].final_feedback_score_threshold
    ) {
      //HIDE BAD
      qsHide(
        "#" +
          this.activities[ancestryIDARR[0]].id +
          " .qs-feedback-final .qs-final-negative-feedback"
      );
      //SHOW GOOD
      qsShow(
        "#" +
          this.activities[ancestryIDARR[0]].id +
          " .qs-feedback-final .qs-final-positive-feedback"
      );
    } else {
      //HIDE GOOD
      qsHide(
  ...
  this.qsAddSubmitEvent = function (activity_id, exercise_id, question_id) {
    ...
    $(
      "#" +
        ...
        this.activities[activity_id].exercises[exercise_id].id +
        ".qs-exercise #" +
        this.activities[activity_id].exercises[exercise_id].questions[
          question_id
        ].id +
        ".qs-question .qs-submit"
    ).attr(
      "aria-controls",
      this.activities[activity_id].exercises[exercise_id].questions[question_id]
        .id
    );
    ...
    if (
      ...
      if (
        ...
        $(
          ...
          if (!$(this).hasClass("qs-disabled")) {
            fCl(
              "---------------------------- PRESSED SUBMIT OVERRIDE GO TO NEXT QUESTION ----------------------------"
            );
            mThis.qsNextQuestion($(this));
          }
        });
      ...
      if (
        ...
        $(
          "#" +
            ...
            ".qs-question .qs-answers input[type=button]"
        ).click(function () {
          if (!$(this).hasClass("qs-disabled")) {
            fCl(
              "---------------------------- PRESSED SUBMIT BUTTON TYPE-5 ----------------------------"
            );
  ...
  this.qsManageSecondChance = function (qStrID) {
    ...
    fCl(
      "Should check for qs-chance-X rather than guessing it has through JQ.length"
    );
    if ($("#" + qStrID + " .qs-feedback .qs-second-chance").length) {
      //SECOND CHANCES HOLDER EXISTS
      //HIDE EVERY DIV INSIDE .qs-second-chance
      qsHide($("#" + qStrID + " .qs-feedback .qs-second-chance > div"));
      //CHECK IF FIRST SECOND CHANCE EXISTS
      if (
        $("#" + qStrID + " .qs-feedback .qs-second-chance").attr(
          "data-chance-count"
        ) === undefined
      ) {
        ...
        qsShow(
          $("#" + qStrID + " .qs-feedback .qs-second-chance > div.qs-chance-1")
        );
        return true; //FIRST SECOND CHANCE
      } else {
        //NOT THE FIRST SECOND CHANCE
        ...
        if (
          $("#" + qStrID + " .qs-feedback .qs-chance-" + nxtChanceID).length
        ) {
          $("#" + qStrID + " .qs-feedback .qs-second-chance").attr(
            "data-chance-count",
            nxtChanceID
          ...
          qsShow(
            $(
              "#" +
                qStrID +
                " .qs-feedback .qs-second-chance > div.qs-chance-" +
                nxtChanceID
            )
          );
  ...
  this.qsScrambleAnswers = function (qStrID, qType) {
    ...
    switch (qType) {
      ...
      default:
        ...
        fCe(
          "qsScrambleAnswer() says: Can't randomize question model -> " + qType
        );
        return false;
    }
    ...
    for (i = 0; i < tmpA.length; i++) {
      $("#qsscrambleanswerstemp").after(
        tmpC.splice(Math.floor(Math.random() * tmpC.length), 1)
      );
    }
  ...
  this.qsGenerateEncodedValidationString = function (mode, val, targetSTRID) {
    fCl(
      "qsGenerateEncodedValidationString(" +
        mode +
        "," +
        val +
        "," +
      ...
      case "gonogo": //AKA TRUE||FALSE, ONE CODE PASSES WHILE THE REST FAILS BASED ON X%masterIntTester
        ...
        if (val === 1) {
          ...
          if (encodedData % this.settings.masterIntTester != 0) {
            encodedData +=
              this.settings.masterIntTester -
              (encodedData % this.settings.masterIntTester);
          }
        } else {
  ...
  this.qsTestAgainstEncodedValidationString = function (mode, testString) {
    fCl(
      "qsTestAgainstEncodedValidationString(" + mode + "," + testString + ")"
    );
  ...
  this.qsGetRandomQuestionStack = function (activity_id, exercise_id) {
    ...
    if (recallInstead) {
      ...
      for (
        ...
        tmpArrB[i] = tmpArr.splice(
          Math.floor(Math.random() * tmpArr.length),
          1
        );
      }
      return tmpArrB;
  ...
  this.qsResetActivity = function (activity_id) {
    fCl(
      "qsResetActivity(" +
        activity_id +
        ") -> NOTE THAT RESETING AN ACTIVITY WILL NOT RESET THE RANDOM ORDER OF QUESTIONS!"
    );
    var i, ii, iii;
    ...
    for (i = 0; i < this.activities[activity_id].nb_exercises; i++) {
      ...
      for (
        ii = 0;
        ii < this.activities[activity_id].exercises[i].nb_questions;
        ii++
      ) {
        this.activities[activity_id].exercises[i].questions[
          ii
        ].dv.my_answer_id = "";
        this.activities[activity_id].exercises[i].questions[ii].dv.status =
          "unattempted";
      }
    }
    ...
    $(
      ...
      .prop("disabled", false)
      .removeClass("qs-disabled qs_good_answer qs_wrong_answer")
      .val("");
    //TYPE-4
    $("#" + this.activities[activity_id].id + " select").val("0");
    $("#" + this.activities[activity_id].id + " select")
      .prop("disabled", false)
      .removeAttr("disabled");
  ...
  this.qsComputeParentStatuses = function (pID) {
    ...
    for (i = 0; i < this.activities[pID].exercises.length; i++) {
      ...
      for (
        ii = 0;
        ii < this.activities[pID].exercises[i].questions.length;
        ii++
      ) {
        fCl(
          "Qstatus:" + this.activities[pID].exercises[i].questions[ii].dv.status
        );
        if (
          this.activities[pID].exercises[i].questions[ii].dv.status !==
            "completed" &&
          this.activities[pID].exercises[i].questions[ii].dv.status !== "failed"
        ) {
  ...
  this.qsComputeActivityScore = function (activity_id, pOption) {
    ...
    for (i = 0; i < this.activities[activity_id].exercises.length; i++) {
      for (
        ii = 0;
        ii < this.activities[activity_id].exercises[i].questions.length;
        ii++
      ) {
        //Good-working
        ...
        score_so_far +=
          ...
          "completed"
            ? 1 *
              this.activities[activity_id].exercises[i].questions[ii]
                .score_value
            : 0;
        score_total +=
          1 *
          this.activities[activity_id].exercises[i].questions[ii].score_value;
      }
    }
    if (pOption === "cssf") {
      return score_so_far;
  ...
  this.qsComputeProgression = function (activity_id, pOption) {
    ...
    for (i = 0; i < this.activities[activity_id].exercises.length; i++) {
      ...
      if (i < this.activities[activity_id].dv.current_exercise_id) {
        question_answered += this.activities[activity_id].exercises[i]
          .nb_questions;
      }
    }
    if (pOption === "qat") {
      return (
        question_answered +
        this.activities[activity_id].dv.current_question_id +
        1
      );
  ...
  this.qsUpdateScoreDisplay = function (activity_id) {
    ...
    //
    //Get current score
    $("#" + this.activities[activity_id].id + " .qs-get-current-score").text(
      this.qsComputeActivityScore(activity_id, "cssf")
    );
    //Get max possible score
    $("#" + this.activities[activity_id].id + " .qs-get-max-score").text(
      this.qsComputeActivityScore(activity_id, "mxsc")
    );
    //Get score in %
    $("#" + this.activities[activity_id].id + " .qs-get-score-on100").text(
      this.qsComputeActivityScore(activity_id, "perc")
    );
    //Get current question
    $("#" + this.activities[activity_id].id + " .qs-get-current-question").text(
      this.qsComputeProgression(activity_id, "qat")
    );
    //Get total question nb
    $("#" + this.activities[activity_id].id + " .qs-get-nb-questions").text(
      this.qsComputeProgression(activity_id, "qtot")
    );
  };
  ...
  this.qsGetExerciseIDFromStringID = function (stringID, current_activity_ID) {
    ...
    if (current_activity_ID == -1 || isNaN(current_activity_ID)) {
      fCl(
        "ERR in qsGetExerciseIDFromStringID('" +
          stringID +
          "' ," +
          current_activity_ID +
          ")  <-- Cannot accept param(s)!"
      );
      return false;
    }
    fID = -1;
  ...
  this.qsGetQuestionIDFromStringID = function (
    stringID,
    current_activity_ID,
    current_exercise_ID
  ) {
    if (
      ...
      isNaN(current_activity_ID) ||
      isNaN(current_exercise_ID)
    ) {
      fCl(
        "ERR in qsGetQuestionIDFromStringID('" +
          stringID +
          "' ," +
          current_activity_ID +
          ", " +
    ...
    fID = -1;
    for (igq in this.activities[current_activity_ID].exercises[
      current_exercise_ID
    ].questions) {
      if (
        this.activities[current_activity_ID].exercises[current_exercise_ID]
          .questions[igq].id == stringID
      ) {
        fID = igq;
        break;
      }
  ...
  this.qsGetAncestryStringIDS = function (elem) {
    ...
    tArr.splice(
      ...
      0,
      $(elem)
        .closest(".qs-question, .qs-exercise, .qs-elearning-activity")
        .attr("id")
    );
    ...
    tArr.splice(
      0,
      0,
      $(elem).closest(".qs-exercise, .qs-elearning-activity").attr("id")
    );
  ...
  this.qsGetAncestryIDS = function (elem) {
    ...
    if (tArr.length >= 3) {
      //GET QUESTION ID
      tbArr[2] = parseInt(
        this.qsGetQuestionIDFromStringID(tArr[2], tbArr[0], tbArr[1])
      );
    }
    return tbArr;
  };
  ...
  this.qsGetClosestMatch = function (needle, haystack) {
    fCl(
      "qsGetClosestMatch(" +
        ...
        "," +
        haystack +
        ") **WIP NEEDS A RESTRICTION SO DOENST GO OUTSIDE OF CURRENT QS-QUESTION!!! ***WIP NEEDS TO CRAWL THROUGH SIBLINGS TOO!"
    );
    //needle = what to find
    ...
    closestStackArr.push(
      $("#" + haystack).siblings(needle),
      $("#" + haystack)
        .parent()
        .find(needle),
      $("#" + haystack)
  ...
  this.qsGetTrackingString = function (activity_STR_ID) {
    fCl(
      "qsGetTrackingString() -> MIGHT ONLY WORK FOR ONE ACTIVITY PER PAGE FOR NOW!"
    );
    ...
    var nbQs =
      iLMSTrackingString.split(qsMarkerStart).length ==
      iLMSTrackingString.split(qsMarkerEnd).length
        ? iLMSTrackingString.split(qsMarkerStart).length - 1
        : -1;
    ...
    //fCl('qsGT Marker B');
    var nbAct =
      iLMSTrackingString.split(qsActMarkerStart).length ==
      iLMSTrackingString.split(qsActMarkerEnd).length
        ? iLMSTrackingString.split(qsActMarkerStart).length - 1
        : -1;
    //fCl('qsGT Marker C');
    var nbEx =
      iLMSTrackingString.split(qsExMarkerStart).length ==
      iLMSTrackingString.split(qsExMarkerEnd).length
        ? iLMSTrackingString.split(qsExMarkerStart).length - 1
        : -1;

    if (nbAct == -1 || nbEx == -1) {
      fCe(
        "NO OR ERRONEOUS ENTRY FOR QUIZSPAWNER's ACTIVITIES OR EXERCISES... CHECK TRACKING STRING (FOR EVERY STARTING MARKER NEED AN ENDING ONE)"
      );
      return false;
    }
    ...
    if (
      iLMSTrackingString.indexOf(qsMarkerStart) == 0 &&
      iLMSTrackingString.indexOf(qsMarkerEnd) ==
        iLMSTrackingString.length - qsMarkerEnd.length
    ) {
      iLMSTrackingString = iLMSTrackingString
        .split(qsMarkerStart)[1]
        .split(qsMarkerEnd)[0];
    }
  ...
  this.qsRecallData = function (recallMode, params) {
    ...
    for (i = 0; i < nbAct; i++) {
      ...
      if (recallMode == "all") {
        ...
        this.activities[activity_id].dv.status =
          qStatusArr[tmpArr[i].split(tSplt[3] + "s:")[1].split(tSplt[4])[0]];
        //REFRESH ACTIVITY EX_AT AND Q_AT
        this.activities[activity_id].dv.current_exercise_id = parseInt(
          tmpArr[i]
            .split(tSplt[3] + "p:")[1]
            .split(tSplt[4])[0]
            .split("-")[0]
        );
        this.activities[activity_id].dv.current_question_id = parseInt(
          tmpArr[i]
            .split(tSplt[3] + "p:")[1]
            .split(tSplt[4])[0]
            .split("-")[1]
      ...
      if ($("#" + activity_string_id).length == 1) {
        ...
        for (ii = 0; ii < tmpArr[i].split(qsExMarkerStart).length - 1; ii++) {
          ...
          //GET EXERCISE IDS
          exercise_string_id = trackingArr[i][ii]
            .split(tSplt[3] + "i:")[1]
            .split(tSplt[4])[0];
          //exercise_string_id = trackingArr[i][ii].split('[id:')[1].split(']')[0].split(':')[1];
          exercise_id = this.qsGetExerciseIDFromStringID(
            exercise_string_id,
            activity_id
          );
          ...
          if (recallMode == "all") {
            ...
            this.activities[activity_id].exercises[exercise_id].dv.status =
              qStatusArr[
                trackingArr[i][ii].split(tSplt[3] + "s:")[1].split(tSplt[4])[0]
              ];
          }
          ...
          if (recallMode == "qorder" && exercise_id == params[1]) {
            exercise_reorder_list = trackingArr[i][ii]
              .split(tSplt[3] + "r:")[1]
              .split(tSplt[4])[0];
          ...
          exercise_params_string =
            trackingArr[i][ii].split(tSplt[4] + tSplt[1])[0] + tSplt[4];
          question_answers_string =
            tSplt[1] + trackingArr[i][ii].split(tSplt[4] + tSplt[1])[1];
          //SPLIT QUESTIONS
          qArr = question_answers_string
            .split(tSplt[2], question_answers_string.split(tSplt[2]).length - 1)
            .join("|!|")
            .split(tSplt[1])
            .join("")
            .split("|!|");
          for (iii = 0; iii < qArr.length; iii++) {
            ...
            fCl(i + "-" + ii + "-" + iii);
            question_id = this.qsGetQuestionIDFromStringID(
              qArr[iii].split(tSplt[0])[0],
              activity_id,
              exercise_id
            );
            ...
            if (recallMode == "all") {
              ...
              this.activities[activity_id].exercises[exercise_id].questions[
                question_id
              ].dv.my_answer_id = qArr[iii].split(tSplt[0])[2];
              this.activities[activity_id].exercises[exercise_id].questions[
                question_id
              ].dv.status = qStatusArr[qArr[iii].split(tSplt[0])[1]];
              ...
              switch (
                ...
                case "type-0":
                  if (qArr[iii].split(tSplt[0])[2].split(qSplt[1]).length > 1) {
                    for (
                      iv = 0;
                      iv < qArr[iii].split(tSplt[0])[2].split(qSplt[1]).length;
                      iv++
                    ) {
                      $(
                        "#" +
                    ...
                    $(
                      "#" + qArr[iii].split(tSplt[0])[2].split(qSplt[0])[0]
                    ).val(qArr[iii].split(tSplt[0])[2].split(qSplt[0])[1]);
                  }
                  break;
                case "type-1":
                  $("#" + qArr[iii].split(tSplt[0])[2]).prop(
                    "checked",
                    "checked"
                  );
                  break;
                case "type-2":
                  if (qArr[iii].split(tSplt[0])[2].split(",").length > 1) {
                    $(
                      "#" + qArr[iii].split(tSplt[0])[2].split(",").join(", #")
                ...
                default:
                  fCl(
                    "can't recall data on Question ID : " +
                      activity_id +
                      "-" +
                      exercise_id +
                      "-" +
  ...
  this.qsSaveData = function (activity_id) {
    ...
    saveBuff +=
      tSplt[3] +
      "s:" +
      this.activities[activity_id].dv.status.charAt(0) +
      tSplt[4];
    //SAVE POSITION
    saveBuff +=
      tSplt[3] +
      "p:" +
      this.activities[activity_id].dv.current_exercise_id +
      "-" +
      this.activities[activity_id].dv.current_question_id +
      tSplt[4];
    for (i = 0; i < this.activities[activity_id].nb_exercises; i++) {
      ...
      saveBuff +=
        ...
        tSplt[3] +
        "i:" +
        this.activities[activity_id].exercises[i].id +
        tSplt[4];
      //SAVE STATUS
      saveBuff +=
        tSplt[3] +
        "s:" +
        this.activities[activity_id].exercises[i].dv.status.charAt(0) +
        tSplt[4];
      //SAVE REORDER
      saveBuff +=
        tSplt[3] +
        "r:" +
        this.activities[activity_id].exercises[i].random_order +
        tSplt[4];
      for (
        ii = 0;
        ii < this.activities[activity_id].exercises[i].nb_questions;
        ...
        saveBuff +=
          tSplt[1] +
          this.activities[activity_id].exercises[i].questions[ii].id +
          tSplt[0] +
          this.activities[activity_id].exercises[i].questions[
            ii
          ].dv.status.charAt(0) +
          tSplt[0] +
          this.activities[activity_id].exercises[i].questions[ii].dv
            .my_answer_id +
          tSplt[2];
      }
  ...
  this.qsValidateChars = function (checkStr, checkAgainst) {
    ...
    if (
      ...
      checkAgainst === "" ||
      typeof this.settings.illegalCharsList !== "string"
    ) {
    ...
    for (i = 0; i < checkAgainst.length; i++) {
      if (checkStr.indexOf(checkAgainst.charAt(i)) !== -1) {
        checkStr = checkStr
          .split(checkAgainst.charAt(i))
          .join(replaceIllegalCharWith);
    ...
    for (i = 0; i < checkStr.length; i++) {
      if (checkStr.charCodeAt(i) > 255) {
        checkStr =
          checkStr.substr(0, i) +
          replaceIllegalCharWith +
          checkStr.substr(i + replaceIllegalCharWith.length);
  ...
  this.qsValidateSemantic = function () {
    ...
    $(".qs-elearning-activity *[id], .qs-elearning-activity[id]").each(
      function (i, e) {
    ...
    for (var i in idcheckArr) {
      if (idcheckArr[i] == idcheckArr[i - 1]) {
        ...
        throw new Error(
          "All IDs must be unique, found duplicate: " + '"' + idcheckArr[i]
        );
        break;
      }
    }
      ...
      $(this)
        ...
        .each(function () {
          if ($(this).find(".qs-question").length <= 0) {
            ...
            errStack.push(
              'Missing .qs-question in ".qs-elearning-activity .qs-exercise"'
            );
          }
    ...
    $(
      ".qs-elearning-activity, .qs-exercise, .qs-question, .qs-answers input:not(.qs-submit):not(.qs-prevquestion):not(.qs-nextquestion)"
    ).each(function () {
      if ($(this).attr("id") === undefined) {
        all_data_params_valid = false;
        //throw new Error('Missing ID for .qs-elearning-activity/.qs-exercise/.qs-question');
        errStack.push(
          "Missing ID for .qs-elearning-activity/.qs-exercise/.qs-question/.qs-answers input:not(.qs-submit):not(.qs-prevquestion):not(.qs-nextquestion)"
        );
      }
    });
    ...
    $(".qs-question").each(function () {
      switch (
        $(this).closest("*[data-question-type]").attr("data-question-type") ===
        undefined
          ? mThis.objDefVal.question.question_type
          : $(this).closest("*[data-question-type]").attr("data-question-type")
      ) {
        case "type-0":
          if ($(this).find(".qs-answers textarea").length <= 0) {
            ...
            errStack.push(
              'Missing textarea in ".qs-question .qs-answers" with [data-question-type=type-0]'
            );
          }
          break;
        case "type-1":
          if ($(this).find(".qs-answers input[type=radio]").length <= 0) {
            ...
            errStack.push(
              'Missing input[type=radio] in ".qs-question .qs-answers" with [data-question-type=type-1]'
            );
          }
          break;
        case "type-2":
          if ($(this).find(".qs-answers input[type=checkbox]").length <= 0) {
            ...
            errStack.push(
              'Missing input[type=checkbox] in ".qs-question .qs-answers" with [data-question-type=type-2]'
            );
          }
          break;
        case "type-3":
          if ($(this).find(".qs-answers input[type=text]").length <= 0) {
            ...
            errStack.push(
              'Missing input[type=text] in ".qs-question .qs-answers" with [data-question-type=type-3]'
            );
          }
          break;
        case "type-4":
          if ($(this).find(".qs-answers select").length <= 0) {
            ...
            errStack.push(
              'Missing select in ".qs-question .qs-answers" with [data-question-type=type-4]'
            );
          }
          break;
        case "type-5":
          if (
            $(this).find(".qs-answers input[type=button]:not(.qs-submit)")
              .length <= 0
          ) {
            all_data_params_valid = false;
            //throw new Error('Missing select in ".qs-question .qs-answers" with [data-question-type=type-5]');
            errStack.push(
              'Missing select in ".qs-question .qs-answers" with [data-question-type=type-5]'
            );
          }
          break;
        default:
    ...
    if (
      all_ids_unique &&
      all_elements_accounted_for &&
      all_data_params_valid &&
      all_logic_appear_ok
    ) {
      return true;
    } else {
  ...
  this.qsReturnRandomArr = function (iNb) {
    ...
    for (rRi = 0; rRi < iNb; rRi++) {
      randMatrixArr[rRi] = tmpMatrixArr.splice(
        Math.floor(Math.random() * tmpMatrixArr.length),
        1
      );
    }
  ...
  function fQsEvDp(evt, pars) {
    fCl("fQsEvDp(" + evt + ")");
    if (
      typeof fQsEventDispatcher !== "undefined" &&
      typeof fQsEventDispatcher === "function"
    ) {
  ...
  if (
    $(window).triggerHandler("Framework:systemInfo") === undefined &&
    typeof masterStructure === "undefined"
  ) {
    //WITHOUT FRAMEWORK
...
$(document).ready(function () {
  ...
  for (i = 0; i < $(".qs-elearning-activity").length; i++) {
    for (j = 0; j < wetComponentLookupArr.length; j++) {
      if (
        $(".qs-elearning-activity").eq(i).parents(wetComponentLookupArr[j][0])
          .length > 0
      ) {
        ...
        switch (wetComponentLookupArr[k][0]) {
          case ".wb-tabs":
            $(".wb-tabs").on("wb-ready.wb-tabs", function (evt) {
              wetComponentRequiredToLoad--;
              if (
                typeof qsObj === "undefined" &&
                wetComponentRequiredToLoad === 0
              ) {
                qsObj = new qsBeta(
                  "qsObj",
                  typeof qsObjParams !== "undefined" ? qsObjParams : {}
